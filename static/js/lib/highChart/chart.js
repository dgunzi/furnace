...
defaultOptions.exporting = {
	...
	//filename: 'chart',
	type: 'image/png',
	url: '/sxisImp/hightchartsDownload',
	width: 800,
	buttons: {
		exportButton: {
			//enabled: true,
...
/*Highcharts.getOptions().colors = Highcharts.map(Highcharts.getOptions().colors, function(color) {
	...
	return {
		        ...
		        stops: [
		            [0, color],
		            [1, Highcharts.Color(color).brighten(-0.3).get('rgb')]  darke		        ]
		    };
		});*/
...
//日志图表
function showChart(container,dataObj,funObj)
{
	var funs = {
		toolTip:function(){
...
{
	var opt = {data:null,obj:null,chartName:'pie',chartTitle:'pie',height:200,funs:null,index:false}
	...
	if(typeof data == 'string')
	{
		...
		for(i=0;i<data.length;i++)
		{
			if(typeof data[i].color != 'undefined')
			{
				...
				var thisColor = data[i].color;
				data[i].color = {
					radialGradient: { cx: 0.5, cy: 0.3, r: 0.7 },
	                stops: [
	                    [0,thisColor],
	                    [1,Highcharts.Color(thisColor).brighten(-0.4).get('rgb')]
	                ]
				}
			}
		}
                ...
                events:{
            		click:function(e){
            			if(opt.index){
            				...
            				if(runStatus!='noData'){
            					window.open('itDetails_getMore.action?itCatalog='+strType+'&runStatus='+runStatus);
            				}
            			}
            		}
                }
