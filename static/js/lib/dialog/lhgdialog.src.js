...
lhgdialog.fn = lhgdialog.prototype =
{
	...
	close: function()
	{
		...
		//if(!(that.config.id == 'Confirm' && that.config.alert)){
		that.unlock();
		//}
...
lhgdialog.confirm = function( content, yes, no, parent)
{
	...
	if((yes && yes.toString().indexOf('.alert(')!=-1) || (no && no.toString().indexOf('.alert(')!=-1)){
		flag = true;
	}*/
 ...
 */
lhgdialog.tips = function( content, time, icon, callback,lock)
{
